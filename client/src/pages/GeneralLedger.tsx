...
import { Plus, Eye, Edit, Trash2, BookOpen, Calculator, Filter, Printer, Loader2 } from "lucide-react";
...
export default function GeneralLedger() {
  ...
  const filteredJournalEntries = useMemo(() => {
    ...
    return journalEntries.filter((entry: any) => {
      ...
      if (entry.entryDate && (fromDate || toDate)) {
        const entryDate = entry.entryDate.split('T')[0];
  ...
  }, [journalEntries, searchTerm, typeFilter, fromDate, toDate]);
  ...
  const handlePrintReport = () => {
          ...
          <table>
            ...
            <tbody>
              ${filteredJournalEntries.map((entry: any) => `
                <tr>
                  <td>${format(new Date(entry.entryDate), "yyyy-MM-dd")}</td>
    ...
    `;
    globalPrintDocument(printContent, "General_Ledger_Report");
  };
