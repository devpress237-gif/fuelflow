...
export default function InvoiceReceipt() {
  ...
  // Helper function to format currency, assuming it's available in context or imported
  const formatCurrency = (amount: number) => {
      // Replace with your actual currency formatting logic
      // Example: using Intl.NumberFormat
      return new Intl.NumberFormat(undefined, {
          style: 'currency',
          currency: transaction.currencyCode || 'PKR', // Default to PKR if not specified
      }).format(amount);
  };
              ...
              <div>
                ...
                <div className="space-y-1">
                  ...
                  {transaction?.customer?.contactPhone && (
                    <p data-testid="text-customer-phone">
                      Phone: {transaction.customer.contactPhone}
                    </p>
                  )}
                  {transaction?.customer?.contactEmail && (
                    <p data-testid="text-customer-email">
                      Email: {transaction.customer.contactEmail}
                    </p>
                  )}
                ...
                <div className="space-y-1 text-sm">
                  <p>
                    ...
                    <span data-testid="text-cashier-name">
                      {transaction?.user?.fullName || 'Unknown Cashier'}
                    </span>
                  </p>
                  <p>
                    <span className="font-semibold">Currency:</span>{' '}
                    <span data-testid="text-currency">
                      {transaction?.currencyCode || 'PKR'}
                    </span>
                  </p>
                  {transaction?.notes && (
                    <p>
                      <span className="font-semibold">Notes:</span>{' '}
                      <span data-testid="text-notes">{transaction.notes}</span>
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="border rounded-lg">
                ...
                {transaction.items && transaction.items.length > 0 ? (
                      transaction.items.map((item: any, index: number) => {
                        // Try to get product from the item first, then from products array
                        ...
                        return (
                          <div key={item?.id || index} className="grid grid-cols-12 gap-4 p-4 border-b last:border-b-0" data-testid={`row-item-${index}`}>
                            <div className="col-span-5">
                              <div className="font-semibold" data-testid={`text-product-name-${index}`}>{product?.name || 'Product'}</div>
                              {product?.hsnCode && (
                            ...
                            <div className="col-span-2 text-right" data-testid={`text-quantity-${index}`}>
                              {parseFloat(item?.quantity || '0').toFixed(3)} {product?.unit || 'L'}
                            </div>
            ...
            <div className="flex justify-end mb-8">
              <div className="w-full max-w-sm space-y-2">
                <div className="flex justify-between text-lg font-semibold">
                  ...
                  <span data-testid="text-total-amount">{formatCurrency(parseFloat(transaction.totalAmount ?? '0'))}</span>
                </div>
                {parseFloat(transaction.paidAmount ?? '0') > 0 && (
                  <div className="flex justify-between text-green-600">
                    <span>Amount Paid:</span>
                    <span data-testid="text-paid-amount">{formatCurrency(parseFloat(transaction.paidAmount || '0'))}</span>
                  </div>
                )}
                {parseFloat(transaction.outstandingAmount ?? '0') > 0 && (
                  <div className="flex justify-between text-red-600 font-semibold">
                    <span>Outstanding:</span>
                    <span data-testid="text-outstanding-amount">{formatCurrency(parseFloat(transaction.outstandingAmount ?? '0'))}</span>
                  </div>
                )}
              </div>
            </div>
