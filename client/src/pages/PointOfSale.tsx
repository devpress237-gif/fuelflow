...
import { Plus, Trash2, Receipt, FileText, Calendar, User, X } from "lucide-react";
...
export default function PointOfSale() {
  ...
  const [newEntityType, setNewEntityType] = useState<'customer' | 'supplier'>('customer');
  const [newEntityData, setNewEntityData] = useState({ name: '', contactPhone: '', type: 'credit' });

  const addEntityMutation = useMutation({
    mutationFn: async (data: any) => {
      const url = newEntityType === 'customer' ? '/api/customers' : '/api/suppliers';
      const response = await apiRequest('POST', url, data);
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.message || 'Failed to add entity');
      }
      return response.json();
    },
    onSuccess: (data) => {
      toast({ title: "Success", description: `${newEntityType} added successfully` });
      queryClient.invalidateQueries({ queryKey: [newEntityType === 'customer' ? "/api/customers" : "/api/suppliers"] });
      setAddEntityDialogOpen(false);
      setNewEntityData({ name: '', contactPhone: '', type: 'credit' });
  ...
  const handleAddEntity = () => {
    ...
    addEntityMutation.mutate(newEntityData);
  };
  ...
  const onSubmit = (data: SaleFormData) => {
    ...
    const saleData = {
      transaction: {
        ...
        paidAmount: data.paymentMethod === "credit" ? "0" : totalAmount.toFixed(2),
        outstandingAmount: data.paymentMethod === "credit" ? totalAmount.toFixed(2) : "0",
      },
      items: Array.from(cart).map(item => ({
        productId: item.productId,
    ...
    console.log("Submitting saleData:", JSON.stringify(saleData, null, 2));
              ...
              <DialogContent>
                ...
                <div className="space-y-4 py-4">
                  <div className="space-y-2">
                    <Label>Type</Label>
                    <Select value={newEntityType} onValueChange={(v: any) => setNewEntityType(v)}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-2">
                    <Label>Phone</Label>
                    <Input 
                      value={newEntityData.contactPhone} 
                      onChange={(e) => setNewEntityData(prev => ({ ...prev, contactPhone: e.target.value }))}
                  ...
                  {newEntityType === 'customer' && (
                    <div className="space-y-2">
                      ...
                      <Select 
                        ...
                        onValueChange={(v: any) => setNewEntityData(prev => ({ ...prev, type: v }))}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                ...
                <div className="flex justify-end gap-2">
                  <Button variant="outline" onClick={() => setAddEntityDialogOpen(false)}>Cancel</Button>
                  <Button onClick={handleAddEntity} disabled={addEntityMutation.isPending}>
