              ...
              <table style="width: 100%; border-collapse: collapse;">
                <thead>
                  <tr style="background: #f3f4f6;">
                    ...
                    <th style="padding: 10px; text-align: right; border: 1px solid #e5e7eb;">Quantity</th>
                ...
                <tbody>
                  ${order.items.map(item => `
                    <tr>
                      ...
                      <td style="padding: 10px; text-align: right; border: 1px solid #e5e7eb;">${parseFloat(item.quantity).toFixed(2)}</td>
                      <td style="padding: 10px; text-align: right; border: 1px solid #e5e7eb;">${formatAmount(parseFloat(item.unitPrice), order?.currencyCode || 'PKR')}</td>
                      <td style="padding: 10px; text-align: right; border: 1px solid #e5e7eb;">${formatAmount(parseFloat(item.totalPrice), order?.currencyCode || 'PKR')}</td>
                    </tr>
                  `).join('')}
                </tbody>
              </table>
                    ...
                    <tbody>
                      {order.items.map((item: any, index: number) => (
                        <tr key={index} className="border-t">
                          ...
                          <td className="text-right p-3">{parseFloat(item.quantity).toFixed(2)}</td>
                          <td className="text-right p-3">{formatAmount(parseFloat(item.unitPrice), order.currencyCode || 'PKR')}</td>
                          <td className="text-right p-3">{formatAmount(parseFloat(item.totalPrice), order.currencyCode || 'PKR')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
