...
export default function StockManagement() {
  ...
  const executeStockAudit = async () => {
    ...
    try {
      for (const [tankId, physicalCount] of auditEntries) {
        ...
        const physicalStock = parseFloat(physicalCount as string);
        ...
        if (Math.abs(difference) >= 0.01) {
          await batchStockMovementMutation.mutateAsync({
            ...
            newStock: physicalStock.toString(),
            unitPrice: "0",
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/tanks", user?.stationId] });
      queryClient.invalidateQueries({ queryKey: ["/api/stock-movements", user?.stationId] });
      queryClient.refetchQueries({ queryKey: ["/api/tanks", user?.stationId] });
      queryClient.refetchQueries({ queryKey: ["/api/stock-movements", user?.stationId] });
    } catch (error) {
      toast({
        title: "Audit Failed",
        description: error instanceof Error ? error.message : "Failed to complete stock audit",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      {/* Stock Alerts & Actions - Full Width */}
      <Card>
        <CardHeader>
          ...
          <form onSubmit={(e) => {
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto">
              <p className="text-sm text-muted-foreground">
                Enter the physical stock count for each tank. The system will automatically adjust for any discrepancies.
              </p>
              {tanks.map(tank => {
                const product = products.find(p => p.id === tank.productId);
                const currentStock = parseFloat(tank.currentStock || "0");
                  ...
                  <div key={tank.id} className="border rounded-lg p-4">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="text-right">
                        ...
                        <div className="font-medium">{currentStock.toFixed(2)}L</div>
                      </div>
                    </div>
                    <div>
                      <label className="text-sm font-medium">Physical Count (Liters)</label>
                      <Input
                        type="number"
                        step="0.01"
                        defaultValue={currentStock.toFixed(2)}
                        onChange={(e) => {
                          const value = e.target.value;
                          auditForm.setValue(tank.id, value);
                        }}
                        className="mt-2"
            ...
            <div className="flex justify-end space-x-3 pt-4">
              <Button type="button" variant="outline" onClick={() => setAuditDialogOpen(false)} data-testid="button-cancel-audit">
                Cancel
              </Button>
              <Button
                type="submit"
